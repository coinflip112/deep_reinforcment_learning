...
class GymEnvironment:
            ...
            if tiles:
                ...
                for separator in separators:
                    self._separator_offsets.append(
                        0
                        if len(separator) <= 1
                        else (separator[1] - separator[0]) / tiles
                    )
                    self._separator_tops.append(
                        math.inf
                        if len(separator) <= 1
                        else separator[-1] + (separator[1] - separator[0])
                    )
        ...
        if self._separators is not None:
            ...
            if self._tiles:
                ...
                for t in range(1, self._tiles):
                    ...
                    states.append(
                        self._first_tile_states
                        + (t - 1) * self._rest_tiles_states
                        + state
                    )
                observation = states
            else:
    ...
    def states(self):
        ...
        elif hasattr(self._env.observation_space, "spaces") and all(
            hasattr(space, "n") for space in self._env.observation_space.spaces
        ):
    ...
    def actions(self):
        if hasattr(self._env.action_space, "n"):
            ...
            raise RuntimeError(
                "The environment has continuous action space, cannot return number of actions"
            )
    ...
    def action_ranges(self):
        if not hasattr(self._env.action_space, "shape"):
            raise RuntimeError(
                "The environment does not have continuous actions, cannot return action ranges"
            )
    ...
    def reset(self, start_evaluate=False):
        if self._evaluating_from is not None and not self._episode_ended:
            raise RuntimeError(
                "Cannot reset a running episode after `start_evaluate=True`"
            )
    ...
    def step(self, action):
        ...
        if done:
            ...
            if self.episode % 10 == 0:
                print(
                    "Episode {}, mean 100-episode return {:.2f} +-{:.2f}".format(
                        self.episode,
                        np.mean(self._episode_returns[-100:]),
                        np.std(self._episode_returns[-100:]),
                    ),
                    file=sys.stderr,
                )
            if (
                self._evaluating_from is not None
                and self.episode >= self._evaluating_from + 100
            ):
                print(
                    "The mean 100-episode return after evaluation {:.2f} +-{:.2f}".format(
                        np.mean(self._episode_returns[-100:]),
                        np.std(self._episode_returns[-100:]),
                        file=sys.stderr,
                    )
    ...
    def parallel_init(self, environments):
        ...
        for i in range(environments):
            ...
            worker = multiprocessing.Process(
                target=GymEnvironment._parallel_worker,
                args=(self, self._env.spec.id, 43 + i, connection_worker),
            )
            worker.start()
    ...
    def parallel_step(self, actions):
        if self._workers is None:
            raise RuntimeError(
                "The parallel_init method was not called before parallel_step"
            )
