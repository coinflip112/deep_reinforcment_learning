...
class Network:
    def __init__(self, env, args):
        ...
        action_diffs = np.array(
            [action_highs[i] - action_lows[i] for i in range(len(action_highs))]
        )

        action_diffs = tf.constant(value=action_diffs, dtype=np.float32)
        action_lows = tf.constant(value=action_lows, dtype=np.float32)

        inputs = tf.keras.layers.Input(shape=env.state_shape)
        x = tf.keras.layers.Dense(args.hidden_layer, activation="relu")(inputs)
        output = tf.keras.layers.Dense(action_components, activation="sigmoid")(x)
